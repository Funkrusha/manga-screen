/******************************************************************************
* COFFEEPAD - a program that makes perfect handbrew.
...
void LcdInitialise(void){
	...
	LcdWrite(LCD_C, 0xB0); //Set LCD Vop (Contrast): Try 0xB1(good @ 3.3V) or 0xBF if your display is too dark
...
void LcdWrite(char dc, char data){
	int i;
	if(dc == 1) // Set Data/Command pin
		P1OUT |= PIN_DC;
	else
		P1OUT &= ~PIN_DC;

	P1OUT &= ~PIN_SCE; // Select Chip Enable low
	...
	P1OUT |= PIN_SCE; // Select Chip Enable High
...
int main(void){
	...
	LcdInitialise();
	LcdClear();
	LcdString("Hoerte jeg noen si nostalgi? ALT var ikke bedre i gamledager :D");
}
